export class SearchUI {
  ...
  renderSearchList(result, keywords) {
    ...
    items.forEach((item, idx) => {
      if (idx === 0) {
        this.searchViewContainer.appendChild(
          this.renderDivider(item.year, item.month)
        )
      } else if (
        items[idx - 1].year !== item.year ||
        items[idx - 1].month !== item.month
      ) {
        this.searchViewContainer.appendChild(
          this.renderDivider(item.year, item.month)
        )
      ...
      this.searchViewContainer.appendChild(
        this.renderItemBlock(item.fileName, item.date, item.preview, keywords)
      )
    })
  }
  ...
  clearSearchList() {
    while (this.searchViewContainer.childElementCount) {
      this.searchViewContainer.removeChild(
        this.searchViewContainer.firstElementChild
      )
    }
  }
            ...
            document.dispatchEvent(
              new CustomEvent('delete-diary', {
                detail: {
                  ...
                  callback: () => {
                    document.dispatchEvent(
                      new CustomEvent('search-keyword-changed')
                    )
                  },
                },
              })
