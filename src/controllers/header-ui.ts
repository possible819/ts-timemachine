...
export class HeaderUI {
  constructor() {
    ...
    this.hiddenDatepicker.addEventListener('change', (event) => {
      ...
      if (
        year !== parseInt(window.stdDateUtil.getYearStr()) ||
        month !== parseInt(window.stdDateUtil.getMonthStr())
      ) {
    ...
    if (!this.weatherIcon) {
      ...
      this.weatherIcon.addEventListener('click', () => {
        if (!this.hiddenWeatherCombo) {
          this.hiddenWeatherCombo = document.querySelector(
            '#hidden-weather-combo'
          )
            ...
            switch (event.currentTarget.value) {
              ...
              case CONST.WEATHER.TYPES[0]:
                this.weatherIcon.setAttribute(
                  'src',
                  './assets/images/w-sunny.svg'
                )
                break
              case CONST.WEATHER.TYPES[1]:
                this.weatherIcon.setAttribute(
                  'src',
                  './assets/images/w-cloudy.svg'
                )
                break
              case CONST.WEATHER.TYPES[2]:
                this.weatherIcon.setAttribute(
                  'src',
                  './assets/images/w-rainy.svg'
                )
                break
              case CONST.WEATHER.TYPES[3]:
                this.weatherIcon.setAttribute(
                  'src',
                  './assets/images/w-snowy.svg'
                )
                break
            }
          })
