...
import { Themes, LocalStorage, ThemeObj } from '../constants'

export class AuthUI {
  private pwdDisplayText: HTMLSpanElement | null
  private lockerSvgObject: HTMLObjectElement | null
  private pwdUnits: HTMLDivElement[]
  private changableButton: HTMLButtonElement | null
  ...
  constructor() {
    ...
    this.lockerSvgObject = document.getElementById(
      'auth-locker-img'
  ...
  private registerEventListeners(): void {
    this.numPadButtons.forEach((numPadButton: HTMLButtonElement) => {
      numPadButton.onclick = this.numPadClicked.bind(this)
    })

    document.addEventListener('show-pwd-change', (event: CustomEventInit) => {
      this.readySetupNewPwd()
      this.successCallback = event.detail.successCallback
      this.cancelCallback = event.detail.cancelCallback
    })

    document.addEventListener('show-pwd-check', (event: CustomEventInit) => {
      this.readyCheckAuth()
      this.successCallback = event.detail.successCallback
    })

    document.addEventListener('auth-checked', (event: CustomEventInit) => {
      const result = event.detail
      if (result && this.successCallback) {
  ...
  loadLockerSvg() {
    ...
    this.lockerSvgObject?.addEventListener('load', function () {
      const themeName = localStorageUtil.getItem(LocalStorage.THEME, 'basic')
      const currentTheme: ThemeObj = Themes.find(
        (theme: ThemeObj) => theme.name === themeName
      ) as ThemeObj
